...
/**
 * This class contains methods for handling AES encryption and decryption.
 *
 *  ONLY USE saltShaker method in this  class. Other encryption/decryption
 *  methods not stable. Please use Crypto class methods instead.
 * @author Akers
 */
public class AES {
	 ...
	 * 
	 * @param c1
	 *            the first response Color
	 * @param c2
	 *            the second response Color
	 * @param c3
	 *            the third response Color
	...
	public static byte[] saltShaker(String password, int c1, int c2, int c3) {
		...
		/* low 4 bits */
		salt[4] = (byte) (password.getBytes()[password.length() - 1] + password
				.getBytes()[0]);
	...
	/**
	 * @param password
	 *            a char[] representing the password used in key creation
	 * @param salt
	 *            a byte[] representing the salt used in key creation
	 * @param encryptedData
	 *            a byte[] representing the encrypted data
	 * @param algorithm
	 *            a String representing the algorithm for decrypting
	 * @return a byte[] represented the decrypted message
	 */
	public static byte[] decrypt(char[] password, byte[] salt,
			byte[] encryptedData, String algorithm) {
...
//	public static byte[] encrypt2(byte[] raw, byte[] encrypted)
//			throws Exception {
...
//	}
//
//	public static byte[] decrypt2(byte[] salt, byte[] encrypted)
//			throws Exception {
...
//	}
	 ...
	 *            a char[] representing the password used in key creation
	 * @param salt
	 *            a btye[] representing the salt used in key creation
	 * @param plainText
	 *            a byte[] representing the data to encrypt
	 * @param algorithm
	 *            a String representing the algorithm for ebcrypting
	 * @return a byte[] represented the encrypted message.
	 */
	public static byte[] encrypt(byte[] plainText) {// char[] password, byte[]
													// salt, byte[] plainText,
		// String algorithm) {
		...
		try {
			...
			byte[] keyBytes = new byte[] { 0x73, 0x2f, 0x2d, 0x33, (byte) 0xc8,
					0x01, 0x73, 0x2b, 0x72, 0x06, 0x75, 0x6c, (byte) 0xbd,
					0x44, (byte) 0xf9, (byte) 0xc1, (byte) 0xc1, 0x03,
					(byte) 0xdd, (byte) 0xd9, 0x7c, 0x7c, (byte) 0xbe,
					(byte) 0x8e };
			...
			cEnc.init(Cipher.ENCRYPT_MODE,
					new SecretKeySpec(keyBytes, "DESede"), new IvParameterSpec(
							ivBytes));
